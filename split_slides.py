...
script_content = """
    <style>
        /* --- DYNAMIC ISLAND (НАВИГАЦИЯ СВЕРХУ) --- */
        .dynamic-island {
            position: fixed;
            top: 20px;
            left: 50%;
    ...
    <script>
        ...
        // Path to the provided sequence frames requires ../ relative since it's in slides folder now
...
for idx, f in enumerate(files):
    ...
    if body_match:
        ...
        # We need to construct an individual HTML page for each slide with the same head
...
<body class="bg-background-dark font-display text-slate-100 overflow-x-hidden selection:bg-primary/30 m-0">
    <div class="relative z-20 bg-background-dark h-auto min-h-screen">
        <section class="w-full min-h-screen relative flex flex-col shrink-0 overflow-hidden bg-background-dark">
            {body_content}
        </section>
    </div>
</body>
...
'''
        ...
        with open(f"slides/slide_{idx + 1}.html", "w", encoding="utf-8") as out:
            out.write(slide_html)

# And now we write the intro video slide standalone in slides/slide_intro.html
intro_file_html = f'''<!DOCTYPE html>
<html class="dark" lang="ru" style="scroll-behavior: smooth;">
<head>
{head_content}
</head>
...
with open("slides/slide_intro.html", "w", encoding="utf-8") as out:
    out.write(intro_file_html)
